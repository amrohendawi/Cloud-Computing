...
QEMU_SSH_IP=$(sudo virsh domifaddr instance-2 | awk '/ipv4/{print $4}' | rev | cut -c 4- | rev)
if [ -z "$QEMU_SSH_IP" ]
then
...
do
echo "uplink" > ${VM_type}-results.csv 
done
...
ssh -i id_rsa ubuntu@$KVM_SSH_IP "sudo apt-get update && sudo apt-get -y install sysbench clang iperf3 make bc"              # make sure the dependencies are installed
scp -i id_rsa benchmark.sh  ubuntu@$KVM_SSH_IP:~/
for _ in $(seq 1 $LOOP_END);
do
    echo "loop $_"
...
ssh -i id_rsa ubuntu@$QEMU_SSH_IP "sudo apt-get update && sudo apt-get -y install sysbench clang iperf3 make bc"              # make sure the dependencies are installed
scp -i id_rsa benchmark.sh  ubuntu@$QEMU_SSH_IP:~/
for _ in $(seq 1 $LOOP_END);
do
    echo "loop $_"
