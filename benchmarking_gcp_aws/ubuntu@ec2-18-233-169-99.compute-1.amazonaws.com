...
echo ">>>>> collecting timeseries.. this should take approx. 30 seconds to finish <<<<<"
D=$(date '+%s')
...
S3=$(sysbench fileio --time=60 --file-test-mode=rndrd --file-num=1 --file-total-size=1G --file-extra-flags=direct run | grep -oP 'reads/s:\s*\K.*')
S4=$(sysbench fileio --time=60 --file-test-mode=seqrd --file-num=1 --file-total-size=1G --file-extra-flags=direct run | grep -oP 'reads/s:\s*\K.*')
